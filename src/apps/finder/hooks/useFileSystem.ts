...
const UUID_MIGRATION_KEY = "zios:indexeddb-uuid-migration-v1";
...
export function useFileSystem(
  ...
  const formatFileSystem = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("zios:file-size-timestamp-sync-v1");
  ...
  useEffect(() => {
    const syncFileSizesAndTimestamps = async () => {
      ...
      const syncKey = "zios:file-size-timestamp-sync-v1";
