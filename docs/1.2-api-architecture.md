...
ZiOS uses Vercel Edge Functions for its backend API, with AI providers, Redis caching, and real-time features.
...
tools: {
  launchApp: {
    description: "Launch ZiOS application",
    parameters: z.object({
...
```typescript
// api/_utils/aiPrompts.ts
export const CORE_PRIORITY_INSTRUCTIONS = `
  You are Zi, an AI assistant in ZiOS...
`;

export const ZI_PERSONA_INSTRUCTIONS = `
  Zi's personality and background...
`;
