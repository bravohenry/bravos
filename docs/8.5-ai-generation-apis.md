...
ZiOS provides several AI-powered endpoints for content generation, separate from the main chat interface. These endpoints power specialized features like applet generation, time-travel web pages, and music metadata extraction.
...
```bash
curl -X POST https://os.zi.lu/api/applet-ai \
...
```bash
curl -X POST "https://os.zi.lu/api/ie-generate?url=apple.com&year=1984" \
  -H "Content-Type: application/json" \
  -d '{
    "messages": [
      {
...
```bash
curl -X POST https://os.zi.lu/api/parse-title \
  -H "Content-Type: application/json" \
  -d '{
...
- **[API Reference](/docs/api-reference)** - Overview of all ZiOS API endpoints
