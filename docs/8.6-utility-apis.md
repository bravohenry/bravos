...
ZiOS provides several utility API endpoints for common operations like fetching link previews, checking iframe embeddability, sharing applets, and admin operations. These endpoints are implemented as Vercel Edge Functions for optimal performance.
...
{
  "id": "abc123def456...",
  "shareUrl": "https://os.zi.lu/applet-viewer/abc123def456...",
  "updated": false,
  "createdAt": 1704067200000
}
```
...
Requires admin authentication (user "zi" with valid token).
...
Administrative endpoints for user management in the chat system. All operations require admin authentication (user "zi" with valid token).
...
Permanently delete a user account (cannot delete admin "zi").
...
Ban a user from the chat system (cannot ban admin "zi").
