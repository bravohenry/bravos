...
The main AI chat endpoint for ZiOS that powers the Chats application. This endpoint handles conversational AI interactions with the "Zi" persona, supports multiple AI providers, and includes extensive tool calling capabilities for controlling the ZiOS system.
...
Anonymous users are identified by IP address. The special user "zi" (with valid token) has unlimited access.

## Tool Calling

The Chat API supports extensive tool calling for controlling ZiOS applications and system features.

### Application Control

#### `launchApp`
Launch an application in ZiOS.
...
#### `generateHtml`
Generate an HTML applet for ZiOS.
...
2. **Dynamic System Prompt** (per-request):
   ...
   - Time information (Zi's time + user's local time)
