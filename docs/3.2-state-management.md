...
ZiOS uses Zustand for state management with 21+ stores following consistent patterns, combined with localStorage and IndexedDB for persistence.
...
|-------|-----------------|---------|
| `useAppStore` | `zios:app-store` | Window/instance management, boot state, AI model selection |
| `useFilesStore` | `zios:files` | Virtual filesystem metadata |
| `useThemeStore` | `os_theme` (manual) | OS theme selection |
| `useDisplaySettingsStore` | `zios:display-settings` | Wallpaper, shaders, screen saver |
| `useDockStore` | `dock-storage` | Dock pinned items, scale, visibility |
| `useAudioSettingsStore` | `zios:audio-settings` | Volume controls, audio feature toggles |
| `useLanguageStore` | `zios:language` | UI language preference |

### App-Specific Stores

| Store | Persistence Key | Purpose |
|-------|-----------------|---------|
| `useChatsStore` | `zios:chats` | Chat rooms, messages, authentication |
| `useIpodStore` | `zios:ipod` | Music library, playback state, lyrics |
| `useVideoStore` | `zios:videos` | Video library, playback |
| `useKaraokeStore` | `zios:karaoke` | Karaoke session state |
| `useTerminalStore` | `zios:terminal` | Command history, vim state |
| `useSynthStore` | `zios:synth` | Synthesizer presets |
| `useSoundboardStore` | `zios:soundboard` | Soundboard slots and recordings |
| `useAppletStore` | `applet-storage` | Applet window dimensions |
| `useFinderStore` | `zios:finder` | Finder instances, view preferences |
| `useTextEditStore` | `zios:textedit` | Recent documents |
| `useInternetExplorerStore` | `zios:ie` | Favorites, history |
| `usePaintStore` | `zios:paint` | Tool settings |
| `usePhotoBoothStore` | `zios:photobooth` | Recent photos |
| `usePcStore` | `zios:pc` | Virtual PC state |
...
export const useExampleStore = create<ExampleState>()(
  persist(
    ...
    {
      name: "zios:example",           // localStorage key
